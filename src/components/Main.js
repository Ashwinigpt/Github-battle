                ...
                <div className=' flex-48 profile flex justify-between align-center'>
                    <div className='flex justify-between align-center'>
                        <div className='img-box'>
                            <img src={this.state.user1data.avatar_url} className='full-width' />
                        </div>
                        <h4>{this.state.user1data.login}</h4>
                    </div>
                    <i onClick={() => { this.handleUser1Presence() }} class="far fa-times-circle"></i>
                </div>
            </>);
        } else {
            return (
                <>
                    <form className=' flex-48 flex justify-between align-center' onSubmit={(event) => this.handleUser1Submit(event)}>
                        <input value={this.state.username1} onChange={(event) => this.handleUser1Input(event)} id='player-1' className='flex-69' type='text' placeholder='Enter Github Username..' />
        ...
        if (this.state.isUser2present === true && this.state.user2data) {
            return (<>
                <div className=' flex-48 profile flex justify-between align-center'>
                    <div className='flex justify-between align-center'>
                        <div className='img-box'>
                            <img src={this.state.user2data.avatar_url} className='full-width' />
                        </div>
                        <h4>{this.state.user2data.login}</h4>
                    </div>
                    <i onClick={() => { this.handleUser2Presence() }} class="far fa-times-circle"></i>
                </div>
            </>);
        } else {
            return (
                <>
                    <form className='flex-48 flex justify-between align-center' onSubmit={(event) => this.handleUser2Submit(event)}>
                        <input value={this.state.username2} onChange={(event) => this.handleUser2Input(event)} id='player-2' className='flex-69' type='text' placeholder='Enter Github Username..' />
                        <input className='flex-30' type='submit' value='Submit' />
                    </form>
                </>
            );
            ...
            <div className='container'>
                <Switch>
                    ...
                    <Route path='/battle' exact>
                        <Battle data={this.state} handleUser1Visibility={() => this.handleUser1Visibility()} handleUser2Visibility={() => this.handleUser2Visibility()} />
                    </Route>
